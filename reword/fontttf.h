...
public:
	...
	Rect put_text_right(Surface *s, int y, const char *textstr, const SDL_Color &textColour, bool bShadow = false);
	Rect put_number(Surface *s, int x, int y, unsigned int number, const char *format, const SDL_Color &textColor, bool bShadow = false);
	Rect put_number(Surface *s, int y, unsigned int number, const char *format, const SDL_Color &textColor, bool bShadow = false);
	void setShadowColour(SDL_Color &c);
	Rect calc_text_metrics(const char *textstr, bool bShadow = false, int x=0, int y=0) const;	//slow as it needs to render internally first
