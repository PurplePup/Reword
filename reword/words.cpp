...
CWords::CWords() :
	m_bList(false), m_bDebug(false), m_wordFile("")
{
	reset();
}

CWords::CWords(const std::string &wordFile) :
	m_bList(false), m_bDebug(false), m_wordFile(wordFile)
{
	load(wordFile);	//calls reset() etc
}

void CWords::clearCurrentWord()
{
	m_word.clear();
	m_wordsInTarget.clear();
	for (int i=0; i<4; m_nWords[i++]=0);
}
...
void CWords::reset()
{
	...
	for (int i=DIF_EASY; i<DIF_MAX; m_nInLevel[i++]=0);
...
bool CWords::splitDictLine(std::string text, DictWord &dictword)
{
...
bool CWords::load(std::string wordFile, unsigned int rndSeed)
{
	if (!wordFile.length() && !m_wordFile.length())
	{
	...
	if (wordFile.length()) m_wordFile = wordFile;	//save it for any reload
	
	std::string lnwrd;
	DictWord dictWord;
	
	std::ifstream ifs1 (m_wordFile.c_str(), std::ifstream::in);	//open the file
	if (ifs1.is_open() && !ifs1.eof())
	{
		if (m_bDebug) std::cout << m_wordFile << std::endl;
		
		int len = 0;
		while (std::getline(ifs1, lnwrd))	
		{
			m_total++;
			if (m_bDebug && (m_total%1000==0)) std::cout << "." << std::flush;	//simple progress (1000 more words)
			...
			{
				if (m_bDebug) std::cout << "Ignore (len) " << lnwrd.c_str() << std::endl;
				m_ignored++;
				continue;
			}
			...
			{
				if (m_bDebug) std::cout << "Ignore (\') " << lnwrd.c_str() << std::endl;
				m_ignored++;
				continue;
			}

			if (m_bDebug) std::cout << "Line " << m_total << ": " << lnwrd.c_str() << std::endl;

			dictPair = dictSet.insert(lnwrd);
			if (!dictPair.second)	//not inserted so is a duplicate word
			{
				if (m_bDebug) std::cout << "Line " << m_total << " Duplicate: " << lnwrd.c_str() << std::endl;
				m_ignored++;
				continue;
			}
			...
			m_nInLevel[dictWord._level]++;	//one more in level

			//add word to dict
			if (m_mapAll.find(lnwrd) == m_mapAll.end())			//not found
			{
				m_mapAll[lnwrd] = dictWord;		//so insert it in the ALL word map
				if (6 == len)					//is a 6 letter word 
					m_vect6.push_back(lnwrd);	//so also add to valid 6 letter word vector used for nextWord()
			}
		}
		ifs1.close();
...
//		std::random_shuffle(m_vect6.begin(), m_vect6.end(), m_randInt);
		...
		std::random_shuffle(m_vect6.begin(), m_vect6.end(), g_randInt);
#endif
		//set the 6word iterator to start
		m_vect6_it = m_vect6.begin();
...
bool CWords::wordInWord(const char * shortWord, const char * word6)
{
...
int CWords::findWordsInWord6(tWordMap &shortwords, const char *word6)
{
	int count = 0;
	if (m_bDebug) std::cout << word6 << ": ";
...
//		if (strcmp((*shtwrd).second.c_str(), word6) == 0) continue;	//exclude same word?
		...
		{
			if (m_bDebug) std::cout << (*shtwrd).second._word.c_str() << ", ";
			
			m_wordsInTarget.insert(tWordsInTarget::value_type( (*shtwrd).second._word.c_str(), false ));	//false = each word not "found" yet
			m_nWords[(*shtwrd).second._word.length() - 3]++;
			count++;
		}
	}
	if (m_bDebug) std::cout << std::endl;
	return count;
}


bool CWords::checkCurrentWord6(tWordVect::const_iterator &it)
{
	bool bOk = true;

	//fill in counters...
	findWordsInWord6(m_mapAll, (*it).c_str()); //side effect - fills m_nWords[]
	int i3 = m_nWords[0];
	int i4 = m_nWords[1];
	int i5 = m_nWords[2];
	int i6 = m_nWords[3];
...
//	if (!i3 || !i4 || !i5 || !i6)	//if any 3 or 4 or 5 letter words missing, exclude the 6 letter word from the game
	{
		if (m_bDebug) std::cout << "Missing : " << (*it).c_str() << " 3:" << i3 << " 4:" << i4 << " 5:" << i5 << " 6:" << i6 << std::endl;
		bOk = false;
	}
...
//	if (i3 + i4 + i5 + i6 > 32)		//this is to be used when diff length word columns wrap on the reword game screen
	{
		if (m_bDebug) std::cout << "Too many : " << (*it).c_str() << " 3:" << i3 << " 4:" << i4 << " 5:" << i5 << " 6:" << i6 << std::endl;
		bOk = false;
	}
...
bool CWords::nextWord(std::string &retln, eDifficulty level,  eGameMode mode, bool reloadAtEnd /*=true*/)
{
	bool bOk = true;
	int failsafe = m_vect6.size(); //size of the whole vector, so allow to loop once to find next
	...
	do
	{
		...
		mapit = m_mapAll.find((*m_vect6_it).c_str());	//find word from vect in the word6 dict map
		if (mapit != m_mapAll.end())					//found, so check the level
		{
			if ( (bOk = ((*mapit).second._level <= (int)level)) )//make sure word is at or below current difficulty level
			{
				if ( (bOk = checkCurrentWord6(m_vect6_it)) )
				{
					...
					if (GM_REWORD != mode)
					{
						...
						m_nWords[0] = m_nWords[1] = m_nWords[2] = 0;
					}
				}
			}
		}
		++m_vect6_it;	//next word
	} while (--failsafe && !bOk);

	if (!bOk) m_word._word = "XXXXXX";	//err in word list - too many or missing
...
int CWords::checkWordsInTarget(std::string &testWord)
{
	//it could still be a 3, 4, 5, or another 6 letter word!
	tWordsInTarget::iterator it = m_wordsInTarget.find(testWord);
	if (it != m_wordsInTarget.end())	//found
	{
...
DictWord CWords::getDictForWord(std::string &wrd)
{
	tWordMap::iterator it = m_mapAll.find(wrd);
	if (it == m_mapAll.end())
	{
